...
import type { ProjectState, Payload, PayloadId } from "./types";
 ...
 */
export async function rehydrateBlobs(state: ProjectState): Promise<ProjectState> {
  if (!hasIDB()) return state;
  // Collect payload IDs whose URIs need rehydration
  const toResolve: { pid: PayloadId; key: string }[] = [];
  for (const [pid, p] of Object.entries(state.payloads)) {
    if (p.uri.startsWith(IDB_REF_PREFIX)) {
      toResolve.push({ pid: pid as PayloadId, key: p.uri.slice(IDB_REF_PREFIX.length) });
    }
  }
  ...
  try {
    ...
    for (const { pid, key } of toResolve) {
      ...
      if (value) {
        patched.payloads[pid] = { ...patched.payloads[pid], uri: value } as Payload;
      }
    }
    db.close();
    return patched;
...
export function saveProjectState(state: ProjectState): void {
  ...
  const offloads: { pid: PayloadId; uri: string }[] = [];
  let slim = state;
  for (const [pid, p] of Object.entries(state.payloads)) {
    if (p.uri.length > BLOB_THRESHOLD && p.uri.startsWith("data:")) {
      offloads.push({ pid: pid as PayloadId, uri: p.uri });
    }
  }
  ...
  if (offloads.length > 0) {
    ...
    for (const { pid } of offloads) {
      slimPayloads[pid] = { ...slimPayloads[pid], uri: `${IDB_REF_PREFIX}${pid}` } as Payload;
    }
