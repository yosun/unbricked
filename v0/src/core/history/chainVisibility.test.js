...
import { createSpaceAIHistory, ensureHistoryGraphForSlice, addOpResultToGraph, getSeedPathIds, getPathHeadStateId, getAncestryPath, serializeAIHistory, deserializeAIHistory, } from "./historyGraph";
describe("chain visibility bug", () => {
    it("shows all states after 2 sequential edits on same seed path", () => {
        let h = createSpaceAIHistory();
        h = ensureHistoryGraphForSlice(h, "s1", { image: "img0" });
        let g = h.slices["s1"];
    ...
    it("shows all states after 3 sequential edits", () => {
        let h = createSpaceAIHistory();
        h = ensureHistoryGraphForSlice(h, "s1", { image: "p0" });
        let g = h.slices["s1"];
        const results = [];
        for (let i = 0; i < 3; i++) {
