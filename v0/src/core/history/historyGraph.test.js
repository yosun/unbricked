...
import { createSpaceAIHistory, ensureHistoryGraphForSlice, addOpResultToGraph, getSeedPathIds, getPathIdForState, getPathHeadStateId, getAncestryPath, getChildStates, getOpEdgeBetween, setDisplayCursor, setOperationCursor, recomputeCaches, serializeAIHistory, deserializeAIHistory, makeStateId, makeOpId, } from "./historyGraph";
...
describe("createSpaceAIHistory", () => {
    ...
    it("creates history with document source image", () => {
        const h = createSpaceAIHistory("payload_abc");
...
describe("ensureHistoryGraphForSlice", () => {
    it("creates a new slice graph with root state", () => {
        const h = createSpaceAIHistory();
        const updated = ensureHistoryGraphForSlice(h, "slice1", { image: "p1" });
    ...
    it("is idempotent for existing slice", () => {
        const h = createSpaceAIHistory();
        const h1 = ensureHistoryGraphForSlice(h, "slice1", { image: "p1" });
        const h2 = ensureHistoryGraphForSlice(h1, "slice1", { image: "p2" });
...
describe("addOpResultToGraph", () => {
    function makeTestGraph() {
        const h = createSpaceAIHistory();
        const updated = ensureHistoryGraphForSlice(h, "s1", { image: "img0" });
    ...
    it("throws for invalid input state", () => {
        const graph = makeTestGraph();
        expect(() => addOpResultToGraph(graph, {
            inputStateId: "nonexistent",
            opType: "img2img",
            outputAssets: [{ image: "x" }],
        })).toThrow("not found");
    });
});
...
describe("recomputeCaches", () => {
    it("rebuilds seedPathIds and pathHeadStateIds", () => {
        ...
        const r1 = addOpResultToGraph(g, {
            inputStateId: g.rootStateId,
            opType: "img2img",
            outputAssets: [{ image: "a" }],
        });
        const r2 = addOpResultToGraph(r1.graph, {
            inputStateId: r1.newStateIds[0],
            opType: "bgRemove",
            outputAssets: [{ image: "b" }],
        });
...
describe("serialize / deserialize", () => {
    it("round-trips SpaceAIHistory", () => {
        const h = createSpaceAIHistory();
        const h1 = ensureHistoryGraphForSlice(h, "s1", { image: "p1" });
        const g = h1.slices["s1"];
        const r = addOpResultToGraph(g, {
            inputStateId: g.rootStateId,
            opType: "img2img",
            outputAssets: [{ image: "p2" }],
