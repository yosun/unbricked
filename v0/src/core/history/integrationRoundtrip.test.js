...
import { createSpaceAIHistory, ensureHistoryGraphForSlice, addOpResultToGraph, serializeAIHistory, getSeedPathIds, getPathHeadStateId, getAncestryPath, SPACE_AI_HISTORY_SCHEMA, } from "./historyGraph";
function putOp(objectKind, id, value) {
    return { op: "put", objectKind, id, value };
}
function newPatch(opts) {
    return {
        id: makeId("patch"),
        kind: "GraphPatch",
 ...
 */
function simulateAiEdit(prev, spaceId, layerIdx, outputPayloadId) {
    const ops = [];
    // 1. getOrCreateSliceIds
    let sliceIds;
    const existingSliceIdsAnn = Object.values(prev.annotations).find((a) => a.target.kind === "Space" && a.target.id === spaceId && a.schema === "ui.layers.sliceIds");
    let sliceIdsAnnId;
    if (existingSliceIdsAnn) {
        sliceIds = JSON.parse(existingSliceIdsAnn.data["ids"]);
        sliceIdsAnnId = existingSliceIdsAnn.id;
    }
    else {
        sliceIds = [makeId("slice")];
    ...
    if (existingHistResult) {
        ...
        ops.push(putOp("Annotation", histAnnId, {
            id: histAnnId,
            kind: "Annotation",
            target: { kind: "Space", id: spaceId },
    ...
    const result = addOpResultToGraph(graph, {
        ...
        opType: "img2img",
        outputAssets: [{ image: outputPayloadId }],
...
describe("full round-trip integration: 2 sequential AI edits", () => {
    it("preserves all states through applyPatch → findAIHistory round-trip", () => {
        ...
        const spaceId = Object.keys(state.spaces)[0];
        // First AI edit
