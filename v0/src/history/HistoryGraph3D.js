...
function useAdaptiveScale(groupRef) {
...
function useZoomToNode() {
    ...
    const startZoom = useCallback((worldPos) => {
        const orbitCtrl = controls;
        zoomRef.current = {
...
function stateThumbUri(state, payloads) {
    if (!state)
        return undefined;
...
function childStates(graph, stateId) {
...
function ancestryPath(graph, stateId) {
    const path = [];
    let cur = stateId;
...
function ThumbNode({ uri, size, position, label, isSelected, isCursor, isOpCursor, is3D, onClick, onDoubleClick, }) {
    ...
    const clickHandlers = useMemo(() => {
        const h = {};
        if (onClick)
            h.onClick = (e) => { e.stopPropagation(); onClick(); };
        if (onDoubleClick)
...
export default function HistoryGraph3D({ graph, payloads, sliceY, prismW, onSelectNode, onSetOperationCursor, documentSourceImageId, onToggleAiPanel, aiEditing, aiPanelOpen, }) {
    ...
    const handleNodeClick = useCallback((stateId) => {
    ...
    const branchSeeds = rootChildren.map((child) => ({ startState: child, forkX: hubX, parentBranchIdx: null }));
    const allBranches = [];
    let seedIdx = 0;
    while (seedIdx < branchSeeds.length) {
        ...
        const chain = [seed.startState];
        let cur = seed.startState;
        const visited = new Set([cur.stateId]);
