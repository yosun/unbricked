...
import type { HistoryGraph, HistoryNode, OpKind } from "./historyTypes";

/** Map core OpType strings to subway-map OpKind. */
function toOpKind(opType: string | undefined): OpKind {
  switch (opType) {
 ...
 */
export function sliceHistoryToSubwayGraph(
  sg: SliceHistoryGraph,
  documentSourceImageId?: string,
): HistoryGraph {
  ...
  const opForState = new Map<string, typeof sg.ops[string]>();
