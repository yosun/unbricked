...
export async function runOperation(op, spaceId, payloadId, ctx, signal) {
    if (op.id === "none") {
...
async function colorizeMask(maskUrl, colorIndex, signal) {
 ...
 */
async function applyMaskToOriginal(maskUrl, originalBitmap, signal) {
    const { blob } = await fetchImageBlob(maskUrl, signal);
...
async function runSam3Segment(op, spaceId, payloadId, ctx, signal) {
    const payload = ctx.state.payloads[payloadId];
    if (!payload)
        throw new Error("Source payload not found");
    ...
    let masks;
    const backgroundIdxSet = new Set();
    const autoSelectedIdxSet = new Set();
    if (isRaw) {
        // Raw mode: use all analyzed masks directly, no filtering or dedup.
        masks = analyzed;
        for (const m of masks)
            autoSelectedIdxSet.add(m.idx);
